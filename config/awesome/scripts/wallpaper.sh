...
Single() {
	...
	if [ "$UseRofi" = "True" ] && [ "$UseFeh" = "True" ]; then
		WallMenu="rofi -dmenu -theme wallpaper -i"
		WallOptions=$(echo -e "fill\ncenter\ntile\nscale\nno-xinerama" | $WallMenu)
	elif [ "$UseRofi" = "True" ] && [ "$UseFeh" = "False" ]; then
		WallMenu="rofi -dmenu -theme wallpaper -i -l 5 -p "Options""
		WallOptions=$(echo -e "zoom\ncenter\ntile\nstretch\nno-randr" | $WallMenu)
	elif [ "$UseRofi" = "False" ] && [ "$UseFeh" = "True" ]; then
