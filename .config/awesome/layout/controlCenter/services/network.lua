    ...
    if value then
        ...
        alright:buttons(gears.table.join(
            	awful.button( {}, 1, function ()
                     awful.spawn("nmcli networking off", false)
                    update_everything(false)
                    require("layout.ding.extra.short")("", "Network disabled")
                    end)
        	)
        )
    else
        ...
        alright:buttons(gears.table.join(
            	awful.button( {}, 1, function ()
                     awful.spawn("nmcli networking on", false)
                     update_everything(true)
                     require("layout.ding.extra.short")("", "Network enabled")
                end)
        	)
        )
    end
