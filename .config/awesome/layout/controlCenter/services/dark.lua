...
awful.spawn.easy_async_with_shell("sed -n 8p $HOME/.config/awesome/theme/ui_vars.lua | awk '{print $3}' | tr -d '\",' "
    , function(stdout)
    ...
    alright:buttons(gears.table.join(
        awful.button({}, 1, function()
            if dark_enabled then
                awful.spawn.with_shell("sed -i '8s/.*/color_scheme        = \"light\",/g' $HOME/.config/awesome/theme/ui_vars.lua"
                    , false)
                awesome.restart()
            else
                awful.spawn.with_shell("sed -i '8s/.*/color_scheme        = \"dark\",/g' $HOME/.config/awesome/theme/ui_vars.lua"
                    , false)
                awesome.restart()
            end
        end)
