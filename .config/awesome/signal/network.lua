-- emits network status (with nmcli)
-- well, it works for me. so yeah
---------------------------------

-- ("signal::network"), function(net_status(bool))
...
local net_cmd = [[
  ...
  awful.widget.watch(net_cmd, update_interval, function(_, stdout)
    ...
    if net_ssid == "disconnected" or net_ssid == "asleep" then
        net_status = false
    end

    -- emit (true or false)
    awesome.emit_signal("signal::network", net_status)
end)
